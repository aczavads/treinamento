...
public interface ContaContabilRepository extends BaseRepository<ContaContabil> {
	...
	List<ContaContabil> recuperarTodasPaginadoManualmente(@Param("page") int page, @Param("size") int size);
...
//	List<ContaContabil> findContasDoPlanoDeContas(Long idPlanoDeContas); 
	...
	@Query(value = "select cc from " + "PlanoDeContas p " + "inner join p.contasContabeis cc "
