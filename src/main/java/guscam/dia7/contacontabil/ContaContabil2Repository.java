...
public interface ContaContabil2Repository extends JpaRepository<ContaContabil2, UUID> {
	...
	@Query(nativeQuery = true, value = "select * from conta_contabil2", countQuery = "select count(*) from conta_contabil2")
	Page<ContaContabil2> recuperarTodas(Pageable pageable);

	@Query(nativeQuery = true, value = "select * from conta_contabil2")
	Slice<ContaContabil2> recuperarTodasFatiadas(Pageable pageable);

	@Query(nativeQuery = true, value = "select * from conta_contabil2 limit :size offset (:page * :size) ")
	List<ContaContabil2> recuperarTodasManualmente(int page, int size);
